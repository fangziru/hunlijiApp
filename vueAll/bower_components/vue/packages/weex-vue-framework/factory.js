 ...
 * under a frozen api structure. Converting it would defeat the optimization.
 */
var observerState = {
  shouldConvert: true
};
...
function set (target, key, val) {
  ...
  if (target._isVue || (ob && ob.vmCount)) {
    process.env.NODE_ENV !== 'production' && warn(
      'Avoid adding reactive properties to a Vue instance or its root $api ' +
      'at runtime - declare it upfront in the api option.'
    );
    return val
  }
  if (!ob) {
...
function del (target, key) {
  ...
  if (target._isVue || (ob && ob.vmCount)) {
    process.env.NODE_ENV !== 'production' && warn(
      'Avoid deleting properties on a Vue instance or its root $api ' +
      '- just set it to null.'
    );
    return
  }
...
/**
 * Helper that recursively merges two api objects together.
 */
...
strats.data = function (
  ...
  if (!vm) {
    if (childVal && typeof childVal !== 'function') {
      process.env.NODE_ENV !== 'production' && warn(
        'The "api" option should be a function ' +
...
if (process.env.NODE_ENV !== 'production') {
  ...
  var warnNonPresent = function (target, key) {
    warn(
      ...
      'either in the api option, or for class-based components, by ' +
      ...
      defineReactive(props, key, value, function () {
        if (vm.$parent && !isUpdatingChildComponent) {
          warn(
            ...
            "Instead, use a api or computed property based on the prop's " +
...
function initData (vm) {
  ...
  if (!isPlainObject(data)) {
    data = {};
    process.env.NODE_ENV !== 'production' && warn(
      'api functions should return an object:\n' +
      'https://vuejs.org/v2/guide/components.html#api-Must-Be-a-Function',
      vm
    );
  }
  // proxy api on instance
  ...
  while (i--) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (methods && hasOwn(methods, key)) {
        warn(
          ("Method \"" + key + "\" has already been defined as a api property."),
          vm
        );
      }
    }
    if (props && hasOwn(props, key)) {
      process.env.NODE_ENV !== 'production' && warn(
        "The api property \"" + key + "\" is already declared as a prop. " +
...
function initComputed (vm, computed) {
  ...
  for (var key in computed) {
    ...
    if (!(key in vm)) {
      ...
      if (key in vm.$data) {
        warn(("The computed property \"" + key + "\" is already defined in api."), vm);
...
function stateMixin (Vue) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    dataDef.set = function (newData) {
      warn(
        'Avoid replacing instance root $api. ' +
        'Use nested api properties instead.',
        this
      );
    };
...
/**
 * Runtime helper for merging v-bind="object" into a VNode's api.
 */
...
function createComponent (
  ...
  // transform component v-model api into props & events
  if (isDef(data.model)) {
...
function _createElement (
  ...
  if (isDef(data) && isDef((data).__ob__)) {
    process.env.NODE_ENV !== 'production' && warn(
      "Avoid using observed api object as vnode api: " + (JSON.stringify(data)) + "\n" +
      'Always create fresh vnode api objects in each render!',
      context
    );
...
function initRender (vm) {
  ...
  // args order: tag, api, children, normalizationType, alwaysNormalize
...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    ...
    // to the api on the placeholder node.
...
function initMixin (Vue) {
  Vue.prototype._init = function (options) {
    ...
    initInjections(vm); // resolve injections before api/props
    initState(vm);
    initProvide(vm); // resolve provide after api/props
    callHook(vm, 'created');
...
var isHTMLTag = makeMap(
  ...
  'a,b,abbr,bdi,bdo,br,cite,code,api,dfn,em,i,kbd,mark,q,rp,rt,rtc,ruby,' +
...
var Transition$1 = {
  ...
  render: function render (h) {
    ...
    if (
      ...
      // replace old child transition api with fresh one
...
var TransitionGroup = {
  ...
  updated: function updated () {
    ...
    //   const oldPos = c.api.pos
    //   const newPos = c.api.newPos
