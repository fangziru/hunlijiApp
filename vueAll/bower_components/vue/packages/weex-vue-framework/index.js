...
/**
 * Create an instance with id, code, config and external api.
...
/**
 * Refresh an instance with id and new top-level component api.
 * It will use `Vue.set` on all keys of the new api. So it's better
...
function createVueModuleInstance (instanceId, weex) {
  ...
  // Hack `Vue` behavior to handle instance information and api
